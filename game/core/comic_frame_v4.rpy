...
image comic_v4_frame = Frame('interface/comic_v2/bg_frame_2.png', Borders(64, 64, 64, 64)) 

init python:
    _v4_kerning  = 5 
    _v4_outlines = [(2, "#000a", 0, 0),]
    _v4_font     = "fonts/mango_comics_er.ttf"

    class ComicFrameV4(renpy.Displayable):
    ...
    _vdd = ComicFrameV4(_trd, plus_y = 2, line_spasing = -2)

    def comic_frame_v4_say(text_list =[],  kerning = 6, size = 45, outlines = [(2, "#000a", 0, 0),], font = "fonts/mango_comics_er.ttf", say_image = "comic_frame_say_3", plus_y = 17, say_pos = ["r", 20], line_spasing = -5, minimum_size_y = 128.0):
        r_list = []
        for i in text_list:
            if type(i) == Text:
                r_list.append(i)
        ...
        return ComicFrameV4(r_list, say_image = say_image, plus_y = plus_y, say_pos = say_pos, line_spasing = line_spasing, minimum_size_y = minimum_size_y)#, plus_y = 2, line_spasing = -2)
        
    i = 2

    comic_frame_v4_predict_list = ["interface/comic_v2/bg_frame_2.png",]

    for i in range(1, 10):

        comic_frame_v4_predict_list.append("interface/comic_v2/comic_frame_say_"+str(i)+".png")
...
label comic_frame_v4_hide_label:
    $ j = renpy.display.core.displayable_by_tag("master", "comic_frame_v4_master")
    $ i = list(j.get_placement()) + [j.xzoom, j.yzoom, j.zoom, j.alpha, j.xanchor, j.yanchor]
    
    $ store.comic_frame_v4_slave  = store.comic_frame_v4_master

    show image comic_frame_v4_master:
        alpha 0.0
        xpos i[0]
        ypos i[1]
        xanchor i[2]
        ...
        xanchor i[11]
        yanchor i[12]
    show image comic_frame_v4_slave:
        xpos i[0]
        ypos i[1]
        xanchor i[2]
        yanchor i[3]
    ...
    return 
label comic_frame_v4_show_label:
#getattr(store, "comic_frame_v4_"+str(main_comic_frame_v4), "#0000")
    
    show image comic_frame_v4_master: #comic_frame_v4_0:
        
        zoom _c_f_v4_zoom
        xpos _c_f_v4_xpos
        ypos _c_f_v4_ypos-50
        xanchor _c_f_v4_xanchor
        yanchor _c_f_v4_yanchor
        alpha .0
        ease .2 alpha 1.0 ypos _c_f_v4_ypos
    $ renpy.pause(.2, hard = True)
    return

label comic_frame_v4_label(
        composite=None, 
        m_xpos = 0, m_ypos=0, 
        hide_label=None,show_label=None):

 #   if not store.comic_frame_v4_swap:

#        $ store.comic_frame_v4_swap = True
    ...
    if hide_label:
        ...
        show image comic_frame_v4_slave:
            ...
            xanchor i[11]
            yanchor i[12]
            easeout .2 ypos i[1]-35 alpha 0.0
        #$ renpy.pause(.2, hard = True)
    $ store.comic_frame_v4_master = composite
...
screen comic_frame_v4_screen():
    
    add _vdd xalign .9 yalign .2



label comic_frame_v4_predict_label(action=renpy.start_predict, images=comic_frame_v4_predict_list):
