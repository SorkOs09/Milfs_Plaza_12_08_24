...
image comic_v3_frame = Frame('interface/comic_v2/bg_frame_2.png', Borders(64, 64, 64, 64)) 

init python:
    _v3_kerning  = 5 
    _v3_outlines = [(2, "#000a", 0, 0),]
    _v3_font     = "fonts/mango_comics_er.ttf"

    class ComicFrameV3(renpy.Displayable):
    ...
    _vdd = ComicFrameV3(_trd, plus_y = 2, line_spasing = -2)

    def comic_frame_v3_say(text_list =[],  kerning = 6, size = 45, outlines = [(2, "#000a", 0, 0),], font = "fonts/mango_comics_er.ttf", say_image = "comic_frame_say_3", plus_y = 17, say_pos = ["r", 20], line_spasing = -5, minimum_size_y = 128.0):
        r_list = []
        for i in text_list:
            if type(i) == Text:
                r_list.append(i)
        ...
        return ComicFrameV3(r_list, say_image = say_image, plus_y = plus_y, say_pos = say_pos, line_spasing = line_spasing, minimum_size_y = minimum_size_y)#, plus_y = 2, line_spasing = -2)
        
    i = 2

    comic_frame_v3_predict_list = ["interface/comic_v2/bg_frame_2.png",]

    for i in range(1, 10):

        comic_frame_v3_predict_list.append("interface/comic_v2/comic_frame_say_"+str(i)+".png")
...
label comic_frame_v3_hide_label:
    ...
    return 
label comic_frame_v3_show_label:
#getattr(store, "comic_frame_v3_"+str(main_comic_frame_v3), "#0000")
    
    show image comic_frame_v3_master: #comic_frame_v3_0:
        
        zoom _c_f_v3_zoom
        xpos _c_f_v3_xpos
        ypos _c_f_v3_ypos-50
        xanchor _c_f_v3_xanchor
        yanchor _c_f_v3_yanchor
        alpha .0
        ease .2 alpha 1.0 ypos _c_f_v3_ypos
    $ renpy.pause(.2, hard = True)
    return

label comic_frame_v3_label(
        composite=None, 
        m_xpos = 0, m_ypos=0, 
        hide_label=None,show_label=None):

 #   if not store.comic_frame_v3_swap:

#        $ store.comic_frame_v3_swap = True
    ...
    if hide_label:
        ...
        #$ renpy.pause(.2, hard = True)
    $ store.comic_frame_v3_master = composite
...
screen comic_frame_v3_screen():
    
    add _vdd xalign .9 yalign .2



label comic_frame_v3_predict_label(action=renpy.start_predict, images=comic_frame_v3_predict_list):
